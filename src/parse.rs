...
impl<T, Dest, M> Parser for Map<T, M>
where
    T: Parser,
    // Dest: Sized,
    M: for<'a> Fn(T::Dest) -> Dest,
{
    type Dest = Dest;
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        self.parser
...
/// Комбинатор, возвращающий результат, который будет успешно
...
where
    A0: for<'a> Parser<Dest = Dest>,
    A1: for<'a> Parser<Dest = Dest>,
{
    type Dest = Dest;
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
fn alt3<'a, Dest: 'a, A0: Parser<Dest = Dest>, A1: Parser<Dest = Dest>, A2: Parser<Dest = Dest>>(
    a0: A0,
    a1: A1,
    a2: A2,
) -> Alt<(A0, A1, A2)> {
...
where
    ...
    A2: for<'a> Parser<Dest = Dest>,
    A3: for<'a> Parser<Dest = Dest>,
    A4: for<'a> Parser<Dest = Dest>,
    A5: for<'a> Parser<Dest = Dest>,
    A6: for<'a> Parser<Dest = Dest>,
    A7: for<'a> Parser<Dest = Dest>,
{
    type Dest = Dest;
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        if let Ok(ok) = self.parser.0.parse(input) {
...
impl Parsable for UserBacket {
    type Parser = Map<
        Delimited<
            All<(StripWhitespace<Tag>, StripWhitespace<Tag>)>,
            Permutation<(KeyValue<Unquote>, KeyValue<<Bucket as Parsable>::Parser>)>,
            StripWhitespace<Tag>,
        >,
    ...
    fn parser() -> Self::Parser {
        map(
            delimited(
                ...
                permutation2(
                    key_value("user_id", unquote()),
                    key_value("backet", Bucket::parser()),
                ),
                strip_whitespace(tag("}")),
            ),
    ...
    fn parser() -> Self::Parser {
        map(
            delimited(
                ...
                permutation2(
                    key_value("user_id", unquote()),
                    key_value("backets", list(Bucket::parser())),
                ),
                strip_whitespace(tag("}")),
            ),
...
impl Parsable for Announcements {
    type Parser = Map<List<<UserBuckets as Parsable>::Parser>, fn(Vec<UserBuckets>) -> Self>;
    fn parser() -> Self::Parser {
...
mod test {
    ...
    #[test]
    fn test_status() {
        let parser = Status::parser();
        assert_eq!(parser.parse("Ok"), Ok(("", Status::Ok)));
        assert_eq!(parser.parse("Ok rest"), Ok((" rest", Status::Ok)));
        assert_eq!(
            parser.parse(r#"Err("something went wrong")"#),
            Ok(("", Status::Err("something went wrong".to_string())))
        );
        assert_eq!(
            parser.parse(r#"Err("esc\"aped")"#),
            Ok(("", Status::Err("esc\"aped".to_string())))
        );
        assert_eq!(parser.parse("Err"), Err(()));
        assert_eq!(parser.parse(""), Err(()));
        assert_eq!(parser.parse("Unknown"), Err(()));
    }

    #[test]
    fn test_as_is() {
        assert_eq!(
            AsIs.parse("hello world"),
            Ok(("", "hello world".to_string()))
        );
        assert_eq!(AsIs.parse(""), Ok(("", "".to_string())));
        assert_eq!(AsIs.parse("123 abc"), Ok(("", "123 abc".to_string())));
    }
    ...
    fn test_backet() {
        ...
        assert_eq!(
            Bucket::parser().parse(r#"Backet{"count":42,"asset_id":"usd",}"#),
            Ok((
                "",
                Bucket {
                    asset_id: "usd".into(),
